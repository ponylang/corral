...
primitive \nodoc\ Execute
  ...
  =>
    try
      ...
      let corral_bin =
        evars.get_or_else(
          corral_bin_key, corral_default)
      let corral_prog =
        Program(h.env, corral_bin)?
      let corral_cmd =
        Action(
          corral_prog, args, h.env.vars)
      ...
      h.fail(
        "failed to create corral Program")
    end
...
class \nodoc\ val DataClone
  ...
  =>
    ...
    let src_root =
      FilePath(auth, "corral/test/testdata")

    _root =
      FilePath.mkdtemp(auth, "test_scratch.")?
  ...
  fun dir_path(subdir: String): FilePath ? =>
    _dir.join(subdir)?
...
primitive RelativePathToPonyc
  """
  Finds ponyc on PATH and returns a relative path
  from the current directory to it.
  """
  fun apply(h: TestHelper): String ? =>
    """
    Search PATH for ponyc and return a relative path.
    """
    var cwd = Path.cwd()
    var ponyc_rel_path: String trn =
      recover trn String.create() end
    ...
    for bindir in
      Path.split_list(path).values()
    do
      let ponyc_path =
        FilePath(
          FileAuth(h.env.root),
          Path.join(bindir, "ponyc"))
