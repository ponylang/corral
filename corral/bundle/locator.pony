...
class val Locator is
  (su.ComparableMixin[Locator] & Hashable & Stringable)
  """
  Encapsulation of the bundle dependency's locator string,
  parsed into distinct fields.
    ...
    for s in suffixes.values() do
      ...
      if parts.size() == 2 then
        try
          ...
          let p = parts(1)?
          bp =
            if p.at("/") then p.trim(1) else p end
          break
        end
      end
    end
    if vs == "" then
      // With no vcs, the locator is the local bundle path
      bp = loc
    end
    repo_path = rp
    vcs_suffix = vs
  ...
  fun compare(that: Locator box): Compare =>
    if repo_path != that.repo_path then
      return repo_path.compare(that.repo_path)
    end
    if vcs_suffix != that.vcs_suffix then
      return vcs_suffix.compare(that.vcs_suffix)
    end
  ...
  fun is_local_vcs(): Bool =>
    is_vcs() and is_local()

  fun is_local_direct(): Bool =>
    not is_vcs() and is_local()
