...
class val Program
  """
  A Program encapsulates an executable program and
  ...
  =>
    ...
    path =
      if Path.is_abs(name) then
        ...
        let cwd = Path.cwd()
        // first try to resolve the binary with the
        ...
        try
          ...
          first_existing(
            file_auth,
            evars.get_or_else(pathkey, ""),
            name)?
        end
      end

  fun tag first_existing(
    auth': FileAuth,
    binpath: String,
    name: String)
    : FilePath ?
  =>
    for bindir in
      Path.split_list(binpath).values()
    do
      ifdef windows then
        let bin_bare =
          FilePath.create(
            auth', Path.join(bindir, name))
        if bin_bare.exists() then
          return bin_bare
        end
        let bin_bat =
          FilePath.create(
            auth', Path.join(bindir, name + ".bat"))
        if bin_bat.exists() then
          return bin_bat
        end
        let bin_ps1 =
          FilePath.create(
            auth', Path.join(bindir, name + ".ps1"))
        if bin_ps1.exists() then
          return bin_ps1
        end
        let bin_exe =
          FilePath.create(
            auth', Path.join(bindir, name + ".exe"))
        if bin_exe.exists() then
          return bin_exe
        end
      else
        let bin =
          FilePath.create(
            auth', Path.join(bindir, name))
        if bin.exists() then
          // TODO: should also stat for executable.
          // FileInfo(bin)
          return bin
        end
      end
...
class val Action
  """
  An Action encapsulates one specific executable action
  with a Program, cli args and env vars.
  """
...
class val ActionResult
  """
  The results of an Action which includes its exit code,
  out and err streams as Strings, and an error message
