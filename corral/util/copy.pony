...
primitive Copy
  ...
  // TODO: Error handling here is sketchy at best
  // because FilePath.walk error handling is
  // non-existent.
  fun tree(
    from_root: FilePath,
    to_root: FilePath,
  ...
  =>
    """
    Copy the `dir_name` tree from `from_root` to
    under `to_root`.
    """
    ...
    from_dir.walk(
      {(dir_path: FilePath,
        dir_entries: Array[String] ref)
      =>
        try
          let path =
            Path.rel(
              from_dir.path, dir_path.path)?
  ...
  =>
    ...
    """
    let from_file =
      match \exhaustive\ OpenFile(from_path)
      | let f: File => f
      | let e: FileErrNo => return e
      end
    let to_file =
      match \exhaustive\ CreateFile(to_path)
      | let f: File => f
      | let e: FileErrNo => return e
      end
