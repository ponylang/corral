...
actor Main
  new create(env: Env) =>
    ...
    let cmd =
      match \exhaustive\ recover val
        CLI.parse(env.args, env.vars)
      end
      | let c: Command => c
    ...
    let ulvl_info =
      if quiet then Warn else Info end
    let uout =
      StringLogger(
    ...
    // Hand off to Executor to resolve dirs
