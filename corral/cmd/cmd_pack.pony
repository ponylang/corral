...
class CmdPack is CmdType
  """
  Packages the project into a corral archive.
  """
  let _output: String
      ...
      try
        ...
        if not path.mkdir() then
          ctx.uout(Error) and ctx.uout.log(
            "pack: unable to create " + _output)
          ctx.env.exitcode(1)
        end

        let car_name: String =
          bundle.info.name + "-"
            + bundle.info.version + ".car"
        let car_path = Path.join(_output, car_name)
        let car_file_path =
          FilePath(project.auth, car_path)
        ...
        for package in sorted_packages.values() do
          encoder.add(
            FilePath.from(project.dir, package)?)?
        end
