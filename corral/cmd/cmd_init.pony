...
class CmdInit is CmdType
  """
  Initializes a new project with empty bundle files.
  """
  ...
  =>
    ctx.uout(Info) and ctx.uout.log(
      "init: in " + project.dir.path)

    // TODO: try to read first to convert/update
    // existing file(s)
    ...
    | let bundle: Bundle =>
      try
        if not ctx.nothing then
          bundle.save()?
          ctx.uout(Info) and ctx.uout.log(
            "init: created: " + bundle.name())
        else
          ctx.uout(Info) and ctx.uout.log(
            "init: would have created: "
              + bundle.name())
        end
      else
        ctx.uout(Error) and ctx.uout.log(
          "init: could not create: "
            + bundle.name())
        ctx.env.exitcode(1)
      end
    | let err: String =>
