...
class CmdRemove is CmdType
  """
  Removes a dependency from the project bundle.
  """
  let locator: String
    ...
    | let bundle: Bundle =>
      ...
      try
        if not ctx.nothing then
          bundle.save()?
          ctx.uout(Info) and ctx.uout.log(
            "remove: removed: " + locator)
        else
          ctx.uout(Info) and ctx.uout.log(
            "remove: would have removed: "
              + locator)
        end
      else
        ctx.uout(Error) and ctx.uout.log(
          "remove: could not update: "
            + bundle.name())
        ctx.env.exitcode(1)
      end
    | let err: String =>
