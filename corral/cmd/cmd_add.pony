...
class CmdAdd is CmdType
  """
  Adds a dependency to the project bundle.
  """
  let locator: String
  let version: String
  let revision: String
  ...
  =>
    ...
    | let bundle: Bundle =>
      try
        let dep =
          bundle.add_dep(locator, version, revision)
        if not ctx.nothing then
          bundle.save()?
          ctx.uout(Info) and ctx.uout.log(
            "add: added dep: "
              + dep.data.json().string() + " "
              + dep.lock.json().string())
          // TODO: maybe just fetch this one new dep
        else
          ctx.uout(Info) and ctx.uout.log(
            "add: would have added dep: "
              + dep.data.json().string() + " "
              + dep.lock.json().string())
        end
      else
        ctx.uout(Warn) and ctx.uout.log(
          "add: could not update " + bundle.name())
        ctx.env.exitcode(1)
      end
    | let err: String =>
