...
class iso _TestEmptyDeps is UnitTest
  """
  Verify that when using a corral.json with empty deps,
  there are never any sync, tag query, or checkout
  operations executed.
  """
...
class iso _TestLocalDirect is UnitTest
  """
  Verify that a bundle with a single local dependency
  executes exactly 1 sync, 1 tag query, and 1 checkout
  operation.
  """
...
class iso _TestMutuallyRecursive is UnitTest
  """
  Verify that when using mutually recursive corral.json
  files that we execute the correct number of operations.
  """
...
class iso _TestRegression120 is UnitTest
  """
  Issue #120 identified a problem with transitive
  dependencies that resulted in too many operations
  being performed across the loading of all dependencies.

  The test as currently constituted, consists of a bundle
  with 2 dependencies. One of those has 2 more in a
  transitive fashion, that should result in 4 syncs and
  corresponding actions happening. However, due to a bug
  in _Updater, it currently does 11.

  With a real VCS like Git, the number that results from
  it is variable based on timing. This test exists to
  prove that issue #120 is fixed and to prevent a similar
...
class iso _TestSelfReferential is UnitTest
  """
  Verify that a self reference in a corral.json results
  in only 1 operation.
  """
...
primitive _OpsRecorderTestRunner
  ...
  =>
    ...
    let log =
      StringLogger(
        Error, h.env.err, SimpleLogFormatter)
    let fp: FilePath =
      _TestData.file_path_from(h, dep_path)?
    let repo_cache = _TestRepoCache(auth)
    let ctx =
      Context(
        h.env, log, log, false, false, repo_cache)
    let project = Project(auth, log, fp)
    let bundle = Bundle.load(fp, log)?
    let vcs_builder: VCSBuilder =
      _TestCmdUpdateVCSBuilder(recorder)
    ...
    // when updater is finished, it will send a
    // `cmd_completed` message to _OpsRecorder which
    // will trigger pass/fail
...
actor _OpsRecorder is CmdResultReceiver
  ...
  be cmd_completed() =>
    _h.assert_eq[U64](
      _expected_sync, _sync)
    _h.assert_eq[U64](
      _expected_tag_query, _tag_query)
    _h.assert_eq[U64](
      _expected_checkout, _checkout)
...
primitive _TestData
  fun file_path_from(
    ...
    : FilePath ?
  =>
    FilePath(FileAuth(h.env.root), "corral/test/testdata")
      .join(subdir)?

primitive _TestRepoCache
  fun apply(auth: FileAuth): FilePath =>
    FilePath(
      auth, "_test_cmd_update_repo_cache")
